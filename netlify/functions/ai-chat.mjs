export default async (req) => {
  ...
  if (!key) {
    return Response.json(
      {
        error: "AI is not configured yet. Add GEMINI_API_KEY in Netlify environment variables."
      },
      { status: 503 }
    );
  }
  ...
  try {
    ...
    const systemInstruction = [
      ...
      "- عند وجود مشكلة شديدة أو مستمرة أو مؤثرة بوضوح على الحياة اليومية، شجعي على استشارة مختص مؤهل.",
      "- عند وجود خطر مباشر على النفس أو الآخرين أو إساءة شديدة، شجعي على طلب المساعدة المهنية أو الطارئة المناسبة.",
      ...
      "- إذا كان مناسباً، اقترحي تمريناً بسيطاً وآمناً للتأمل أو الوعي الذاتي.",
